...
static void BM_std_exp2(benchmark::State &state)
{
    for (auto _ : state)
    {
        ...
        {
            float x = float(i);
            benchmark::DoNotOptimize(std::exp2f(i));
        }
    }
}
BENCHMARK(BM_std_exp2)->Arg(1024);

static void BM_narukami_exp2(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            float x = float(i);
            benchmark::DoNotOptimize(exp2(i));
        }
    }
}
BENCHMARK(BM_narukami_exp2)->Arg(1024);
...
static void BM_reciprocal(benchmark::State &state)
{
    for (auto _ : state)
    {
        ...
        {
            float x = float(i);
            benchmark::DoNotOptimize(1.0f / x);
        }
    }
}
BENCHMARK(BM_reciprocal)->Arg(1024);

static void BM_narukami_rcp(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            float x = float(i);
            benchmark::DoNotOptimize(rcp(x));
        }
    }
}
BENCHMARK(BM_narukami_rcp)->Arg(1024);

static void BM_narukami_safe_rcp(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            float x = float(i);
            benchmark::DoNotOptimize(safe_rcp(x));
        }
    }
}
BENCHMARK(BM_narukami_safe_rcp)->Arg(1024);

static void BM_reciprocal_square_root(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            float x = float(i);
            benchmark::DoNotOptimize(1.0f / narukami::sqrt(x));
        }
    }
}
BENCHMARK(BM_reciprocal_square_root)->Arg(1024);

static void BM_narukami_rsqrt(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
...
BENCHMARK(BM_narukami_rsqrt)->Arg(1024);

static void BM_narukami_rsqrt_quake(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            float x = float(i);
            benchmark::DoNotOptimize(rsqrt_quake(x));
        }
    }
}
...
inline BM_Spectrum operator+(const BM_Spectrum &lhs, const BM_Spectrum &rhs)
{
    BM_Spectrum spd;
    for (int i = 0; i < SPD_SAMPLE_COUNT; ++i)
    {
...
inline bool is_black(const BM_Spectrum &spd)
{
    for (int i = 0; i < SPD_SAMPLE_COUNT; ++i)
    {
...
inline float average(const BM_Spectrum &spd)
{
    float sum;
    for (int i = 0; i < SPD_SAMPLE_COUNT; ++i)
    {
    ...
    return sum/SPD_SAMPLE_COUNT;
}

static void BM_trivial_Spectrum_operator_add(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            BM_Spectrum lhs = float(i),rhs = float(i);
            benchmark::DoNotOptimize(lhs + rhs);
        }
    }
}
BENCHMARK(BM_trivial_Spectrum_operator_add)->Arg(1024);


static void BM_narukami_Spectrum_operator_add(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
...
BENCHMARK(BM_narukami_Spectrum_operator_add)->Arg(1024);

static void BM_trivial_Spectrum_is_black(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            BM_Spectrum lhs = 0.0f;
            benchmark::DoNotOptimize(is_black(lhs));
        }
    }
}
BENCHMARK(BM_trivial_Spectrum_is_black)->Arg(1024);

static void BM_narukami_Spectrum_is_black(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
...
BENCHMARK(BM_narukami_Spectrum_is_black)->Arg(1024);

static void BM_trivial_Spectrum_average(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
        {
            BM_Spectrum lhs = 0.0f;
            benchmark::DoNotOptimize(average(lhs));
        }
    }
}
BENCHMARK(BM_trivial_Spectrum_average)->Arg(1024);

static void BM_narukami_Spectrum_average(benchmark::State &state)
{
    for (auto _ : state)
    {
        for (auto i = 0; i < state.range(0); i++)
...
BENCHMARK(BM_narukami_Spectrum_average)->Arg(1024);
...
//     {
//         for(int i=0;i<state.range(0);++i){
//             benchmark::DoNotOptimize(cross(narukami::Vector3fPack(data[6*i],data[6*i+1], data[6*i+2]),narukami::Vector3fPack(data[6*i+3], data[6*i+4], data[6*i+5])));
//         }
//     }
//     delete [] data;
// }
...
//     {
//         for(int i=0;i<state.range(0);++i){
//             benchmark::DoNotOptimize(dot(narukami::Vector3fPack(data[6*i],data[6*i+1], data[6*i+2]),narukami::Vector3fPack(data[6*i+3], data[6*i+4], data[6*i+5])));
//         }
//     }
//     delete [] data;
// }
...
//     float t;
//     int index;
//     narukami::RayPack r(narukami::Point3f((float)rand(),(float)rand(),(float)rand()),narukami::Vector3f((float)rand(),(float)rand(),(float)rand()));
//     auto triangles = new narukami::TrianglePack[(size_t)state.range(0)]();
//     for(size_t i = 0; i < state.range(0); i++)
//     {
//        triangles[i].v0=narukami::Point3fPack((float)rand(),(float)rand(),(float)rand());
//        triangles[i].e1 = narukami::Vector3fPack((float)rand(),(float)rand(),(float)rand());
//        triangles[i].e2 = narukami::Vector3fPack((float)rand(),(float)rand(),(float)rand());
//     }
...
// //      auto transform = std::make_shared(translate(Vector3f(0,0,0)));
// //      auto transform2 = std::make_shared(translate(Vector3f(0,0,0)));
...
// //                 RayPack ray(Point3f(0.5f,0.5f,0.5f),narukami::Vector3f(0,0,1));
