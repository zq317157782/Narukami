...
int main()
{
...

    {
        shared<Mesh> mesh;
        {
            auto transform = make_shared(translate(Vector3f(0, 0, 1.0f)) * scale(0.01f, 0.01f, 0.01f) /** rotate(90, Vector3f(0, 1, 0))*/);
    ...
    auto blas_to_wrold4 = make_shared(translate(-1.0f, 0, 0));
    auto world_to_blas4 = make_shared(inverse(*blas_to_wrold4));

    {
...

        {
            ...
            auto instance = shared<BLASInstance>(new BLASInstance(make_shared<AnimatedTransform>(blas_to_wrold4), blas));
            instance_list.push_back(instance);
        }
    }
